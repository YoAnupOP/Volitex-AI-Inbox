...
const isAiMode = computed(() => {
  return currentChat.value?.custom_attributes?.ai_mode === true;
});

const toggleAiMode = async () => {
  const newValue = !isAiMode.value;
  try {
    await store.dispatch('toggleAiMode', {
      conversationId: currentChat.value.id,
      aiMode: newValue,
    });
    useAlert(
      newValue
        ? t('CONVERSATION.HEADER.AI_MODE_ENABLED')
        : t('CONVERSATION.HEADER.AI_MODE_DISABLED')
    );
  } catch (error) {
    useAlert(t('CONVERSATION.HEADER.AI_MODE_ERROR'));
  }
};
</script>
...
<template>
  ...
  >
    ...
    >
      ...
      <button
        v-tooltip="isAiMode ? $t('CONVERSATION.HEADER.AI_MODE_ON') : $t('CONVERSATION.HEADER.AI_MODE_OFF')"
        class="flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-md transition-colors"
        :class="
          isAiMode
            ? 'bg-n-violet-3 text-n-violet-11 hover:bg-n-violet-4'
